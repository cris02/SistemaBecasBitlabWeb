...
public class UsuarioManejador extends ManejadorAbstracto<Usuario> {
    ...
    public void guardarEntidad() {
        ...
        if (getEntidadSeleccionada().getUsuIdPk() != null) {
            ...
            pass = "";
            getEntidadSeleccionada().setUsuEstado(true);
            getEntidadSeleccionada().setUsuFechacreacion(new Date());
            getEntidadSeleccionada().setUsuFechamodificacion(new Date());

        }

        String contra = getEntidadSeleccionada().getUsuPass(); //contrasena obtenida del formulario
        //verifica si la contrasena del input del formulario esta vacia
        if (contra.isEmpty()) {
            getEntidadSeleccionada().setUsuPass(pass);
        } else if (!(getEntidadSeleccionada().getUsuPass().equals(pass))) {
            EncriptacionTexto encriptacionTexto = new EncriptacionTexto();
            getEntidadSeleccionada().setUsuPass(encriptacionTexto.getTextoEncriptado(contra));
        }
        //guardar la entidad
    ...
    public void obtenerEstado(Usuario usuario) {
        ...
        this.selectEstado = getEntidadSeleccionada().getUsuEstado(); // se obtiene el estado del input
        getEntidadSeleccionada().setUsuEstado(selectEstado);
        if (getEntidadSeleccionada().getUsuEstado()== selectEstado) {
            super.guardarEntidad();
        } else {
